                 ...
                 int *succ, int *decode )
{
    ...
    code = code_m + n;
		...
		// initialise all adjacencies with 0
    for ( i = 1; i < maxAdj; i++ )
        for ( j = 1; j < maxAdj; j++ )
            adj[i][j] = 0;

		// determine how often two genes are adjacent in the input; 
		// adj[i][j] = number of how often the 'genes' i and j are adj in the input
		// negative genes -i are transformed to n-i : i.e. n+|i|
...
#ifdef MOD
				// if sign > 0 then do not regard adjacencies 
    ...
    for ( i = 1; i < maxAdj; i++ )
    {
		...
		// compose the single element chains to chains
    for ( i = 1; i < maxAdj; i++ )
    {
        if ( chain[i] == NULL )
            endAdj[i] = -1;
	...
	for ( i = -n; i <= n; i++ )
	{
		if( (i < 0 && decode[i]>0) || (i > 0 && decode[i]<0)){
