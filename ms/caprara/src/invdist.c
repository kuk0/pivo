...
void invd_init_hdata(unsigned len, char circ){
	init_data_c(invhd, len, circ);
}

void invd_free_hdata(){
	free_data_c(invhd);
}
...
/**
 * this is the original code of the funtion invdist_noncircular. this extra function was needed
 * because the awkward linking with the rEvoluzer sources :
 * - the original function was modified per define to return interval preserving reversal distances
 * - in the case of common intervals the function srac ist called
 * - this function calls invdist from grappa to get the original reversal distance
 * - but it gets the modified version because the modified version was linked
 * -> problem: endles recursion 
 * -> solution : this extra function is called from srac to break this circle
 * -> and all calls from grappa go to the original (modified) version
 */
int
invdist_noncircular_mb ( struct genome_struct *g1, struct genome_struct *g2,
                      int offset, int num_genes, distmem_t * distmem )
{
...
#ifdef DEBUG
    fprintf ( stderr, "invdist_noncircular: offset: %3d\n", offset );
#endif
    ...
    printf ( "Number of hurdles:     %d\n", num_hurdles );
    printf ( "Fortress:              %d\n", num_fortress );
    printf ( "REVERSAL DISTANCE: %3d\n", reversal_dist );
#endif
                      ...
                      int offset, int num_genes, distmem_t * distmem )
{
	...
	g_off = distmem->perm1; 	// ' misuse ' of distmem, there is enough space
	
	for ( i = 0; i < num_genes; i++ )
	{
		if ( offset < num_genes )
			g_off[i] = g2->genes[( offset + i ) % num_genes];
		else
		{
			g_off[i] = -g2->genes[( offset - i ) % num_genes];
		}
...
#ifdef COM
#ifdef SIG
		reversal_dist = srac_c(g1->genes, g_off, 1, num_genes, 0, invhd);
#else
		reversal_dist = perfect_distance_c(g1->genes, g_off, num_genes, invhd);
#endif//SIG
#endif//COM
#ifdef CON
	printf("invdist_noncircular: a lazy guy commented out preserving_reversal_distance .. \n");
	printf("           so.. this is not usable until some body reenables this\n");
//	reversal_dist = preserving_reversal_distance(g1->genes, g_off, num_genes, 0, invhd);
#endif//CON
	return reversal_dist;
#else//MOD
...
/**
 * see comment to invdist_noncircular_mb
 */
int
invdist_circular_mb ( struct genome_struct *g1, struct genome_struct *g2,
